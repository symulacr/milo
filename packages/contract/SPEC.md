...
Full technical reference for the Milo order contract. The overview and the lifecycle diagram are in
[README.md](README.md).
...
The lifecycle diagram is in [README.md](README.md). Every arrow there is one proof-bearing circuit call, the label gives the required phase guard, and the five timeout and escalation calls invoke no witness.
